...
function mostrarDatosEnPerfil() {
    ...
    let countAlbumes = albumesFamosos.length;
    let totalAlbumText = document.getElementById('cant-albums').innerText=`${countAlbumes} álbumes`

    let countFav = albumesFamosos.filter(album => album.like).length;
    let totalFavAlbumText = document.getElementById('cant-favoritos').innerText=`${countFav} favoritos`
}
